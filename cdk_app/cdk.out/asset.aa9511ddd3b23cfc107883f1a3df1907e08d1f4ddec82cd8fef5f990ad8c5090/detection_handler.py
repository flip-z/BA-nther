...
def detect(event, context):
    ...
    try:
        ...
        if event.get('rawPath', '').endswith('/health') or event.get('path', '').endswith('/health'):
        ...
        # Parse request for detection (skip body check for health checks)
        if not event.get('rawPath', '').endswith('/health') and 'body' not in event:
